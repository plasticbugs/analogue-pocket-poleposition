#!/usr/bin/env python3
"""Build the Pocket .rom image for the Pole Position core from a MAME romset.
...
MAME `polepos` romset -- either the zip or a directory of loose files -- checks
...
Usage:
    ...
    mra_build.py polepos.mra polepos.zip
    mra_build.py timeplt.mra ~/roms/polepos/
"""
...
def build(mra_path, romset_paths, verbose=False):
    ...
    parts = {}
    for path in romset_paths:
        for name, data in load_parts(path).items():
            parts.setdefault(name, data)
    image = bytearray()
    for node in rom:
...
def main():
    ...
    mra = args[0]
    rest = args[1:]
    out = rest.pop() if len(rest) > 1 and rest[-1].lower().endswith('.rom') else None
    romset = rest
    if out is None:
    ...
    if verbose:
        print(f'{mra} + {", ".join(romset)}:')
    image, md5 = build(mra, romset, verbose)
    with open(out, 'wb') as f:
        f.write(image)
    print(f'wrote {out} ({len(image)} bytes, md5 {md5}) - verified')
    print('copy it to  Assets/poleposition/common/  on your Pocket SD card')
